...
static const char * const mkfs_usage[] = {
	...
	OPTLINE("-r, --rootdir DIR", "copy basic files and directories to the image"),
	OPTLINE("--compress ALGO[:LEVEL]", "not supported with --rootdir on OpenBSD"),
	OPTLINE("-u, --subvol TYPE:SUBDIR", "not supported with --rootdir on OpenBSD"),
	OPTLINE("--inode-flags FLAGS:PATH", "not supported with --rootdir on OpenBSD"),
	OPTLINE("--reflink", "not supported with --rootdir on OpenBSD"),
	OPTLINE("--shrink", "not supported with --rootdir on OpenBSD"),
...
static int ensure_regular_file_size(const char *path, u64 size)
{
	struct stat statbuf;
	int fd;
	int ret = 0;
	...
	if (S_ISREG(statbuf.st_mode) && (u64)statbuf.st_size < size &&
	    ftruncate(fd, size) < 0)
		ret = -errno;
out:
	close(fd);
...
int BOX_MAIN(mkfs)(int argc, char **argv)
{
	...
	if (source_dir) {
		...
		if (compression != BTRFS_COMPRESS_NONE) {
			error("--compression with --rootdir is not supported on OpenBSD");
			ret = 1;
			goto error;
		}
		if (do_reflink) {
			error("--reflink is not supported on OpenBSD");
			ret = 1;
			goto error;
		}
		if (shrink_rootdir) {
			error("--shrink is not supported on OpenBSD");
			ret = 1;
			goto error;
		}
		ret = btrfs_mkfs_validate_target(source_dir,
		    argv[saved_optind]);
		if (ret < 0)
			goto error;
	} else {
	...
	ret = btrfs_mkfs_validate_source_dir(source_dir);
	if (ret < 0)
		goto error;
	...
	for (i = 0; i < device_count; i++) {
		...
		if ((source_dir || byte_count_set) && path_exists(file) == 0)
			ret = 0;
	...
	*/
	...
	if (source_dir) {
		...
		if (ret < 0) {
			ret = -errno;
			error("unable to stat %s: %s", file, strerror(-ret));
			close(fd);
			goto error;
		}
		...
		ret = btrfs_mkfs_size_dir(source_dir, sectorsize,
				min_dev_size, metadata_profile, data_profile,
				&source_dir_size);
		if (ret < 0) {
			error("failed to calculate source directory size: %s",
			    strerror(-ret));
			close(fd);
			goto error;
		}
...
"the target device %llu (%s) is smaller than the calculated source directory size %llu (%s), mkfs may fail",
	...
	if (byte_count_set) {
		for (i = saved_optind; i < saved_optind + device_count; i++) {
			ret = ensure_regular_file_size(argv[i], byte_count);
			if (ret < 0) {
				errno = -ret;
				error("unable to create or resize %s: %m", argv[i]);
				goto error;
			}
		}
	}
