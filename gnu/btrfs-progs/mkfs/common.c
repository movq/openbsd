...
static int is_swap_device(const char *file)
{
	...
	if (stat(file, &target) < 0)
		return -errno;
	if (!S_ISBLK(target.st_mode) && !S_ISREG(target.st_mode))
		return 0;
	...
	entries = calloc(nswap, sizeof(*entries));
	if (entries == NULL)
		return -ENOMEM;
	ret = swapctl(SWAP_STATS, entries, nswap);
	if (ret < 0) {
		ret = -errno;
		goto out;
	}
	...
	for (i = 0; i < ret; i++) {
		...
		if (stat(entries[i].se_path, &entry) < 0)
			continue;
		if (S_ISBLK(target.st_mode) && S_ISBLK(entry.st_mode) &&
		    target.st_rdev == entry.st_rdev) {
			ret = 1;
			goto out;
		}
		if (S_ISREG(target.st_mode) && S_ISREG(entry.st_mode) &&
		    target.st_dev == entry.st_dev &&
		    target.st_ino == entry.st_ino) {
			ret = 1;
			goto out;
		}
	}
...
static int check_overwrite(const char *device)
{
	...
	if (fd < 0)
		return -1;
	for (i = 0; i < BTRFS_SUPER_MIRROR_MAX; i++) {
		ret = pread(fd, &sb, sizeof(sb), btrfs_sb_offset(i));
		if (ret == sizeof(sb) && btrfs_super_magic(&sb) == BTRFS_MAGIC) {
			error("%s appears to contain an existing btrfs filesystem",
			    device);
			close(fd);
			return 1;
		}
	...
	close(fd);
	return check_btrfs_signature_zoned(device);
}
