...
volatile sig_atomic_t btrfs_send_stream_cancelled;
...
static int read_buf(struct btrfs_send_stream *sctx, char *buf, size_t len)
{
	...
	while (pos < len) {
		...
		if (rbytes < 0) {
			if (errno == EINTR && !btrfs_send_stream_cancelled)
				continue;
			ret = -errno;
...
static int read_cmd(struct btrfs_send_stream *sctx)
{
	...
	/* Bound allocation before trusting an unverified command header. */
	if (cmd == 0 || cmd > BTRFS_SEND_C_MAX_V1 ||
	    cmd_len > BTRFS_SEND_BUF_SIZE_V1 - sizeof(*cmd_hdr)) {
		ret = -EOPNOTSUPP;
		goto out;
	}
...
out:
	if (ret < 0)
		error("stream command %d%s%s: %s", sctx->cmd,
		    path ? " path " : "", path ? path : "", strerror(-ret));
	free(path);
	free(path_to);
	free(clone_path);
	free(xattr_name);
				       ...
				       u64 max_errors)
{
	...
	if (memcmp(hdr.magic, BTRFS_SEND_STREAM_MAGIC, sizeof(hdr.magic))) {
		ret = -EINVAL;
