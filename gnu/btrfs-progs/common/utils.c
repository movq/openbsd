...
int find_mount_root(const char *path, char **mount_root)
{
	...
	for (i = 0; i < count; i++) {
		if (path_is_in_dir(mntbuf[i].f_mntonname, path)) {
			len = strlen(mntbuf[i].f_mntonname);
			if (longest_matchlen <= len) {
				free(longest_match);
				longest_matchlen = len;
				longest_match = strdup(mntbuf[i].f_mntonname);
				if (!longest_match) {
					ret = -errno;
					break;
				}
				not_btrfs = strcmp(mntbuf[i].f_fstypename, "btrfs");
			}
		}
	}
...
unsigned long total_memory(void)
{
	...
	if (pages < 0 || page_size < 0)
		return -1UL;
	return (unsigned long)pages * page_size;
}
...
static char *sprint_profiles(u64 profiles)
{
	...
	if (profiles & BTRFS_AVAIL_ALLOC_BIT_SINGLE)
		strlcat(ptr, btrfs_raid_array[BTRFS_RAID_SINGLE].lower_name,
		    maxlen);
	...
	for (i = 0; i < BTRFS_NR_RAID_TYPES; i++) {
		...
		if (ptr[0])
			strlcat(ptr, ", ", maxlen);
		strlcat(ptr, btrfs_raid_array[i].lower_name, maxlen);
	}
