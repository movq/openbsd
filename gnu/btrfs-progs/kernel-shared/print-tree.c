				 ...
				 int array_size)
{
	...
	for (i = 0; i < array_size; i++) {
		...
		if ((flag & supported_flags) && (flag & entry->bit)) {
			if (dest[0])
				strlcat(dest, "|", BG_FLAG_STRING_LEN);
			strlcat(dest, entry->output, BG_FLAG_STRING_LEN);
		}
	}
	...
	if (flag) {
		if (dest[0])
			strlcat(dest, "|", BG_FLAG_STRING_LEN);
		snprintf(unknown, sizeof(unknown), "UNKNOWN: 0x%llx", flag);
		strlcat(dest, unknown, BG_FLAG_STRING_LEN);
	}
}
...
static void compress_type_to_str(u8 compress_type, char *ret)
{
	...
	case BTRFS_COMPRESS_ZSTD:
		...
		break;
	default:
		snprintf(ret, 16, "UNKNOWN.%d", compress_type);
	}
}
...
static void print_header_info(struct extent_buffer *eb, unsigned int mode)
{
	...
	if (fs_info && (mode & BTRFS_PRINT_TREE_CSUM_HEADERS)) {
		...
		for (int i = 0; i < csum_size; i++) {
			snprintf(tmp, 3, "%02x", tree_csum[i]);
			tmp++;
			tmp++;
		}
	} else {
